...
import { ClipboardCheck, Search, FileDown, Loader2, School, Calendar, History } from "lucide-react";
...
export default function GuruHistoryPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (resClasses.ok) setClasses(dataClasses);
    } catch (err) {
  ...
  const filteredHistory = history.filter(r => {
    const matchKelas = selectedKelas === "all" || r.santri.kelas.id === selectedKelas;
    const matchSearch = r.santri.name.toLowerCase().includes(searchTerm.toLowerCase()) || r.santri.nis.includes(searchTerm);
  ...
  const handleExport = () => {
    ...
    const exportData = filteredHistory.map(r => ({
      ...
      'Nama Santri': r.santri.name,
      'NIS': r.santri.nis,
      'Kelas': r.santri.kelas.name,
      'Status': r.status,
      'Keterangan': r.notes || '-'
    }));
    ...
    utils.book_append_sheet(workbook, worksheet, "Riwayat Absensi");
    
    let filename = "Riwayat_Absensi_Saya";
    ...
    <DashboardLayout>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500">Daftar absensi yang telah Anda inputkan</p>
        </div>
      ...
      <div className="card mb-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700 ml-1">Filter Kelas</label>
            ...
            <table className="w-full text-left">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-6 py-4 text-sm font-bold text-slate-600 uppercase tracking-wider">Catatan</th>
                </tr>
              </thead>
                ...
                {filteredHistory.length > 0 ? filteredHistory.map((report) => (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600">
                          {report.santri.name.charAt(0)}
                        </div>
                        <div>
                          <p className="font-bold text-slate-800 text-sm leading-tight">{report.santri.name}</p>
                          <p className="text-[10px] text-slate-400 font-medium tracking-wider uppercase">{report.santri.nis}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="bg-blue-50 text-blue-700 px-2.5 py-1 rounded-lg text-xs font-bold border border-blue-100">
                        {report.santri.kelas.name}
                      </span>
                    </td>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-20 text-center">
                      ...
                      <h3 className="text-slate-500 font-bold">Belum Ada Riwayat</h3>
                      <p className="text-slate-400 text-sm mt-1">Absensi yang Anda inputkan akan muncul di sini.</p>
                    </td>
                  </tr>
                )}
              </tbody>
